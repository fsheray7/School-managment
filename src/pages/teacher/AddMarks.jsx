  ...
  return (
    <section className="w-full min-h-screen bg-white flex flex-col items-center overflow-x-hidden pb-6">
      ...
      <div className="w-full mt-8 px-0 sm:px-4">
        {/* Header */}
        <div
          className="flex items-center justify-between border-b py-2 px-6 text-white text-xs sm:text-sm font-bold shadow-sm"
          style={{ backgroundColor: "var(--primary-color)", opacity: 0.6 }}
        >
          <p>Student Name</p>
        ...
        {students.map((student) => (
          ...
          >
            <p className="whitespace-nowrap text-[#000000] font-medium">
              {student.name}
            </p>

            <input
              type="text"
              className="w-12 h-6 border border-gray-300 text-center outline-none transition-all focus:ring-1 focus:ring-[var(--primary-color)]/30"
            />
          </div>
        ))}

        <footer
          className="w-full max-w-5xl border mt-6 px-4 py-3 flex fixed bottom-0 justify-between items-center text-white text-sm sm:text-base font-semibold z-1000 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]"
