    ...
    return (
        <section className="flex flex-col items-center justify-start w-full bg-white min-h-screen gap-4 px-4 pb-10 pt-20">
            ...
            <div className='flex w-full justify-between items-center max-w-6xl'>
                <h1 className="text-lg md:text-2xl font-bold text-[#0C46C4]">Students Management</h1>
                <button onClick={() => navigate("/add-student")} className='border-2 flex  border-[#0C46C4] text-[#0C46C4] px-2 py-1 md:px-4 md:py-1  cursor-pointer  hover:scale-105 transition-all duration-300 rounded-md font-md shadow-md'>
                    + Student
                </button>
            </div>
                ...
                <table className="w-full text-left border-collapse">
                    <thead className="bg-[#E8F8F6] text-[#0C46C4] uppercase text-xs font-bold">
                        <tr>
                            ...
                            <th className="px-6 hidden md:table-cell lg:table-cell py-4">Email</th>
                            <th className="px-6 py-4 hidden md:table-cell">Subject</th>
                            <th className="px-6 py-4 hidden lg:table-cell">Section</th>
                            <th className="px-6 py-4 hidden md:table-cell">Guardian Contact</th>
                    ...
                    <tbody className="divide-y divide-gray-100 text-sm text-gray-700">
                        {studentsData.map((student) => (
                            <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-6 py-4 text-gray-500 hidden md:table-cell lg:table-cell">{student.email}</td>
                                <td className="px-6 py-4 hidden md:table-cell">{student.subject}</td>
                                <td className="px-6 py-4 hidden lg:table-cell">{student.section}</td>
                                <td className="px-6 py-4 hidden md:table-cell">{student.guardianContact}</td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-md relative overflow-hidden animate-fade-in">
                        <div className="bg-[#E8F8F6] px-6 py-4 flex justify-between items-center">
                            <h3 className="text-lg font-bold text-[#0C46C4]">Student Details</h3>
                        ...
                        <div className="p-6 flex flex-col gap-4">
                            <div className="flex flex-col gap-1">
                                ...
                                <span className="text-gray-800 font-medium">{selectedStudent.fullName}</span>
                            </div>
                            <div className="flex flex-col gap-1">
                                <span className="text-xs text-gray-500 uppercase font-bold">Guardian Name</span>
                                <span className="text-gray-800">{selectedStudent.guardianName}</span>
                            </div>
                            <div className="flex flex-col gap-1">
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                <span className="text-gray-800">{selectedStudent.subject}</span>
                            </div>
                            <div className="flex flex-col gap-1">
                                <span className="text-xs text-gray-500 uppercase font-bold">Section</span>
                                <span className="text-gray-800">{selectedStudent.section}</span>
                            </div>
                            <div className="flex flex-col gap-1">
                                <span className="text-xs text-gray-500 uppercase font-bold">Guardian Contact</span>
                                <span className="text-gray-800">{selectedStudent.guardianContact}</span>
                            </div>
                        </div>
